...
public class User {
	...
	public User(String userName,int type){
		...
		try{
			if(type == USER){
				icon = ImageIO.read(getClass().getResource("mrt1.png"));
				statusImage = ImageIO.read(getClass().getResource("status4.png"));
			}
			else{
				icon  = ImageIO.read(getClass().getResource("mrt4.png"));
				statusImage = ImageIO.read(getClass().getResource("status3.png"));
			}
	...
	public void draw(Graphics g){
		...
		g2d.drawImage(statusImage,
				30,MainFrame.HEIGHT-icon.getHeight()-240,null);
		g2d.setColor(Color.BLACK);
		g2d.drawString(Status, 
				90, MainFrame.HEIGHT-icon.getHeight()-210);
